...
import type { Credential, AccessGrant } from '../types/index.js';
...
export class VaultDB {
  ...
  getCredential(namespace: string, name: string): Credential | null {
    const stmt = this.db.prepare('SELECT * FROM credentials WHERE namespace = ? AND name = ?');
    const row = stmt.get(namespace, name) as any;
  ...
  // Grant operations
  createGrant(grant: AccessGrant): void {
    const stmt = this.db.prepare(`
  ...
  hasAccess(agentId: string, credentialId: string): boolean {
    ...
    const result = stmt.get(agentId, credentialId) as any;
