...
program
    ...
    for (const cred of mockCredentials) {
        if (options.namespace && cred.namespace !== options.namespace)
            continue;
...
program
    ...
    // Build credentials map (TODO: load from actual vault DB)
    const credentials = new Map([
        ['personal:chase', { user: 'matt', password: 'secret123' }],
        ['personal:chase.user', { user: 'matt' }],
    ]);
...
program
    ...
    for (const entry of mockAuditEntries.slice(0, parseInt(options.limit))) {
        if (options.credential && !entry.credential.includes(options.credential))
            continue;
...
program
    ...
    .addCommand(new Command('devices')
...
async function question(rl, prompt, hidden = false) {
